...
class SupportUserManager:
    ...
    async def bind_question(
        ...
        return [
            TextToSend(
                await self.messages.get_successful_binding_message(question)
            )
        ]
    ...
    async def unbind_question(self) -> list[MessageToSend]:
        ...
        return [
            TextToSend(await self.messages.get_successful_unbinding_message())
        ]
    ...
    async def add_role(
        ...
        if role:
            return [
                TextToSend(
                    await self.messages.get_role_name_duplicate_message()
                )
            ]
        ...
        return [
            TextToSend(
                await self.messages.get_successful_role_addition_message(
                    new_role
                )
            )
        ]
    ...
    async def get_all_roles(self) -> list[MessageToSend]:
        ...
        return [TextToSend(await self.messages.get_roles_list_message(roles))]
    ...
    async def get_question_answers(
        ...
        return [
            TextToSend(await self.messages.get_answers_list_message(answers))
        ]
